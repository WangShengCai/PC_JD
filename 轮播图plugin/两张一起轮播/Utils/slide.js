(function ($) {
    ...
    function Slider(dom, opt) {
        ...
        // 容器宽度
        this.domWidth = opt.showWidth || $(dom).width();
        // 展示两张图片的数量
        this.domImgNum = Math.floor(this.imgNum * this.width / this.domWidth);
        // 是否自动轮播
    ...
    Slider.prototype.createDom = function () {
        ...
        // 不确定图片数量，需要遍历决定创建多少个li
        this.imgList.forEach(function (item, index) {
            $('<li><a href="#"><img src="' + item + '"></img></a></li>').appendTo(myUl);
        });
        // 判断运动方式，如果是animate，在ul后面在创建一个li图片和第一张是一样的
        if (this.animateType == 'animate') {
            var num = Math.floor(this.domWidth / this.width);
            for (var i = 0; i < num; i++) {
                var myLi = $('<li><a href="#"><img src="' + this.imgList[i] + '"></img></a></li>');
                myUl.append(myLi);
            }
        }
        // 创建左边的按钮
        var leftBtn = $('<a href="javascript:void(false)" class="left-btn">&lt;</a>');
        // 创建右边的按钮
        var rightBtn = $('<a href="javascript:void(false)" class="right-btn">&gt;</a>');
        // 如果是不是一次轮播 两张图片 的情况
        if (this.domWidth == this.width) {
            for (var i = 0; i < this.imgNum; i++) {
                myCircle.append($('<span></span>'));
            }
            myCircle.appendTo(this.dom);
        } else if (this.width * 2 == this.domWidth || this.width * 4 == this.domWidth) {
            for (var i = 0; i < this.domImgNum; i++) {
                myCircle.append($('<span></span>'));
            }
            myCircle.appendTo(this.dom);
        }
    ...
    Slider.prototype.initStyle = function () {
        ...
        if (this.animateType == 'animate') {
            $('.slider', this.dom).css({
                position: 'relative',
                width: this.domWidth * this.domImgNum + this.domWidth,
                height: this.height,
            });
    ...
    Slider.prototype.move = function (dir) {
        ...
        switch (dir) {
            ...
            case 'prev':
                if (this.animateType == 'animate') {
                    if (this.direction == 'left' && this.nowIndex == 0) {
                        $('.slider', this.dom).css({
                            left: -this.domImgNum * this.domWidth,
                        });
                        this.nowIndex = this.domImgNum - 1;
                    } else {
                        if (this.nowIndex == 0) {
                            this.nowIndex = this.domImgNum;
                            $('.slider', this.dom).css({
                                left: -this.domImgNum * this.domWidth,
                            })
                        }
            ...
            case 'next':
                if (this.animateType == 'animate') {
                    if (this.direction == 'right' && this.nowIndex == this.domImgNum) {
                        $('.slider', this.dom).css({
                            left: 0,
                        });
                        this.nowIndex = 1;
                    } else {
                        if (this.nowIndex == this.domImgNum) {
                            this.nowIndex = 1;
                            $('.slider', this.dom).css({
                                left: 0,
                            })
                    ...
                    if (this.nowIndex == this.domImgNum - 1) {
                        this.nowIndex = 0;
                    } else {
                        this.nowIndex++;
                    }
        ...
        // 如果运动方式是animate
        if (this.animateType == 'animate') {
            $('.slider', this.dom).animate({ left: - this.domWidth * this.nowIndex }, 'swing', function () {
                that.flag = false;
            })
        }
        // 如果运动方式是fade
        ...
        $('.circle span', this.dom).css({
            backgroundColor: '#fff',
        }).eq(this.nowIndex % this.domImgNum).css({
            backgroundColor: '#f40',
        });
    }
